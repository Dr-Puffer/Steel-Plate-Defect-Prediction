# Steel-Plate-Defect-Prediction
Classwork for "Advanced Machine Learning Theory" 
  1、tf26.yaml 是环境文件，用于Conda。
  2、all.py测试了逻辑回归、随机森林、XGBoost、LightGBM和Catboost这些机器学习模型的表现，
  3、LightGBM_hyper.py和Catboost_hyper.py文件是对LightGBM和Catboost进行超参数探索的脚本。
  4、LightGBM_train.py和Catboost_train.py是LightGBM和Catboost的训练脚本，生成相关模型以及输出文件。
  5、训练集与测试集请在Kaggle官网下载：https://www.kaggle.com/competitions/playground-series-s4e3
